...
const APPS = [
  ...
  },
  {
    ...
    description: 'La app para barberías y salones que quieren llevar todo en el iPhone. Agenda citas con recordatorios automáticos, controla el stock de productos y genera informes financieros en PDF.',
    iosUrl: 'https://apps.apple.com/pe/app/barbercitas-agenda-y-clientes/id6776738526',
    color: '#f39c12',
...
const WEB_SYSTEMS = [
  {
    ...
    emoji: '💳',
    name: 'GestorPro',
    category: 'Finanzas · Préstamos · Web',
    description: 'Sistema web multidispositivo para prestamistas. Gestiona clientes, préstamos y cobros desde PC, tablet o móvil. Olvídate de Excel y profesionaliza tu negocio.',
    webUrl: 'https://gestor-creditos-webs.vercel.app/',
    color: '#fe2c55',
    highlights: ['Clientes y préstamos', 'Cuotas automáticas', 'Reportes en PDF'],
  },
  {
    id: 2,
    emoji: '🏢',
    name: 'PropiGest',
    category: 'Propiedades · Alquileres · Web',
    description: 'Sistema web para administrar propiedades en alquiler. Controla inquilinos, contratos, rentas, ingresos y egresos desde cualquier dispositivo.',
    webUrl: 'https://gestor-propiedades-pro-etd8.vercel.app/',
    color: '#9b59f5',
    highlights: ['Gestión de inquilinos', 'Contratos digitales', 'Finanzas y reportes'],
  },
]
...
function WebBadge() {
  return (
    <div className="app-card__badge app-card__badge--web">
...
function ProductCard({ product, variant = 'ios' }) {
  const url = variant === 'ios' ? product.iosUrl : product.webUrl
  const ctaLabel = variant === 'ios' ? 'Descargar gratis en App Store' : 'Probar gratis en la web'
  const ariaLabel = variant === 'ios'
    ? `Descargar ${product.name} en App Store`
    : `Abrir ${product.name} en el navegador`

  return (
    <article className="app-card" style={{ '--card-accent': product.color }}>
      <div className="app-card__header">
        <div className="app-card__icon" style={{ background: `${product.color}18`, border: `1.5px solid ${product.color}50` }}>
          <span role="img" aria-label={product.name}>{product.emoji}</span>
        </div>
        {variant === 'ios' ? <IosBadge /> : <WebBadge />}
      </div>
      <div className="app-card__info">
        <span className="app-card__category">{product.category}</span>
        <h3 className="app-card__name">{product.name}</h3>
        <p className="app-card__desc">{product.description}</p>
      </div>
      <ul className="app-card__highlights">
        {product.highlights.map(h => (
          <li key={h}>
            <span className="highlight-dot" style={{ background: product.color }} aria-hidden="true" />
            {h}
          </li>
        ))}
      </ul>
      <a
        ...
        className={`store-btn store-btn--ios-full${variant === 'web' ? ' store-btn--web' : ''}`}
        style={{ '--btn-color': product.color }}
    ...
    <div className="landing">
      ...
      <header className="hero">
        ...
        <div className="hero__content">
          ...
          <p className="hero__subtitle">
            Apps para iPhone y sistemas web para tu negocio. Descarga gratis o prueba en línea y desbloquea funciones premium.
          </p>
      ...
      <section id="apps" className="section">
        ...
        <div className="apps-grid">
          {APPS.map(app => (
            <ProductCard key={app.id} product={app} variant="ios" />
          ))}
        </div>
      </section>

      {/* ── SISTEMAS WEB ── */}
      <section id="web" className="section section--dark">
        <div className="section__header">
          <p className="section__label section__label--cyan">Sistemas web</p>
          <h2 className="section__title">Gestiona tu negocio<br />desde cualquier dispositivo</h2>
          <p className="section__subtitle">Plataformas web multidispositivo. Accede desde PC, tablet o móvil. Plan gratuito para empezar, sin tarjeta de crédito.</p>
        </div>
        <div className="apps-grid">
          {WEB_SYSTEMS.map(system => (
            <ProductCard key={system.id} product={system} variant="web" />
          ))}
        </div>
      </section>
      ...
      <footer className="footer">
        ...
        <div className="footer__links">
          <a href="#apps">Apps iOS</a>
          <span aria-hidden="true">·</span>
          <a href="#web">Sistemas web</a>
          <span aria-hidden="true">·</span>
